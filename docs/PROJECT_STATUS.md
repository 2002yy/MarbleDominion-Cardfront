...
The normal player-facing run uses `core_tactics`, containing exactly sixteen
upgrades:
...
- `core_tactics`: the active default sixteen-card pool.
- `fortification_corps`: an implemented Engineer-oriented candidate containing
  Siege Formation, the four entity cards, Building Volley, and Heavy Charge.
...
- Repair Units, Armored Guard, Fire-Control Beacon, and Interceptor Tower are
  offered by the current formal pool. Later entities remain locked below and require a
...
Next entity-content order is fixed: Sapper Unit, then the first neutral
creature. They must not enter the manifest until their runtime behavior, AI
value, visual readability, and native tests land together.
...
- Next gameplay implementation order: Sapper Unit, then the first neutral
  creature.
