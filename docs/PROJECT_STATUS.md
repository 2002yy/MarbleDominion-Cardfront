...
The normal player-facing run uses `core_tactics`, containing exactly seventeen
upgrades:
...
- `core_tactics`: the active default seventeen-card pool.
- `fortification_corps`: an implemented Engineer-oriented candidate containing
  Siege Formation, the five entity cards, Building Volley, and Heavy Charge.
...
- Repair Units, Armored Guard, Sapper Unit, Fire-Control Beacon, and Interceptor
  Tower are offered by the current formal pool. Later entities remain locked
  below and require a dedicated implementation slice.
...
The next entity-content target is the first neutral creature. It must not enter
the manifest until its identity, allegiance rules, runtime behavior, AI value,
visual readability, and native tests land together.
...
- Next gameplay implementation target: the first neutral creature.
