...
func _test_desktop_layout(grid_size: int) -> void:
	...
	_assert.gte(battlefield_rect.position.y, 88.0, "arena layout: battlefield should leave room for top HUD")
	...
	_assert.gte(arena_view_rect.size.y / viewport_size.y, 0.78, "arena layout: 3D arena should feel vertically expanded")
