...
func _test_state_priority_and_transitions() -> void:
	...
	_assert.that(not actor.sprite.flip_h, "entity animation: right movement keeps source facing")
	actor.sync_cell(Vector2i(1, 2))
	_assert.that(actor.sprite.flip_h, "entity animation: left movement mirrors the same frame set")
	actor.play_attack()
