...
func _test_mixes() -> void:
	...
	_assert.eq(ProjectileTypeScript.territory_pressure_units(ProjectileTypeScript.SUPPRESSION), 0.5, "one suppression shot opens a route but cannot claim a full cell alone")
...
func _test_cards() -> void:
	...
	_assert.eq(ProjectileTypeScript.territory_pressure_units(doubled.projectile_sequence[0]) + ProjectileTypeScript.territory_pressure_units(doubled.projectile_sequence[1]), 1.0, "two copied suppression shots together equal one standard territory unit")
