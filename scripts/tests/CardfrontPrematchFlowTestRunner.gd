...
func _test_battle_identity_hud() -> void:
	...
	_assert.that(hud.ai_stats.text.contains("36/36"), "battle HUD: AI plate should show mirrored chamber health")
	_assert.gte(hud.player_name.get_theme_font_size("font_size"), 16, "battle HUD: hero identity should remain readable")
	_assert.gte(hud.player_stats.get_theme_font_size("font_size"), 12, "battle HUD: health and volley text should remain readable")
