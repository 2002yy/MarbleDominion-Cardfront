...
func _test_cardfront_builds_true_3d_mirror() -> void:
	...
	_assert.that(float(badge_metrics.get("pixel_size", 1.0)) <= 0.013, "orthographic arena: stronghold badge should not cover its platform")
	...
	_assert.gte(view.get_entity_visual_scale_for_test(repair_id), 1.20, "orthographic arena: combat creatures should receive the readability scale")
	_assert.gte(view.get_entity_visual_scale_for_test(tower_id), 1.20, "orthographic arena: defense towers should receive the readability scale")
