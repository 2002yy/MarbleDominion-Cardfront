...
func _test_manifest_is_valid() -> void:
	...
	_assert.eq(UpgradeManifestScript.get_upgrade_ids().size(), 15, "upgrade manifest: formal live pool should contain fifteen upgrades")
	_assert.eq(UpgradeManifestScript.get_all_upgrade_ids().size(), 15, "upgrade manifest: catalog should contain only implemented upgrades")
