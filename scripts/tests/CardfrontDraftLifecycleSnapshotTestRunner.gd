...
func _test_choice_and_next_draft_lifecycle() -> void:
	...
	_assert.that(not panel._peeking, "draft_opened: preview state starts reset")
	...
	await process_frame
	_assert.that(not panel._peeking, "Space: current implementation returns to Draft mode")
	_assert.eq(panel.choice_shell.position, baseline_position, "Space: current implementation restores the saved shell position")
	Input.parse_input_event(space_event)
	await process_frame
	preview_position = panel.choice_shell.position
	_assert.that(panel._peeking, "Space: current implementation can enter preview again")

	director._process(0.25)
	_assert.that(panel._peeking, "timer update: preview flag is preserved")
	...
	_assert.that(panel._peeking, "choices_revealed: current implementation retains preview flag")
	_assert.eq(panel.choice_shell.position, preview_position, "choices_revealed: current implementation retains moved shell")
	...
	_assert.that(panel._peeking, "volley_launched: current implementation does not reset preview flag")
	_assert.eq(panel.choice_shell.position, preview_position, "volley_launched: current implementation does not restore geometry")

	director.force_open_draft_for_test()
	await process_frame
	_assert.that(panel.draft_root.visible, "next draft_opened: DraftRoot becomes visible")
	_assert.that(not panel._peeking, "next draft_opened: preview flag resets")
	_assert.eq(panel._peek_button.text, "查看战场", "next draft_opened: button text resets")
	_assert.eq(panel.choice_shell.position, preview_position, "next draft_opened: current implementation preserves drifted shell position")
	_assert.neq(panel.choice_shell.position, baseline_position, "next draft_opened: current geometry does not return to the golden baseline")
...
func _test_timeout_while_preview_lifecycle() -> void:
	...
	_assert.that(panel._peeking, "timeout preview: current implementation retains preview flag during reveal")
	_assert.eq(panel.choice_shell.position, preview_position, "timeout preview: current implementation retains moved geometry during reveal")
...
func _test_director_stop_lifecycle() -> void:
	...
	_assert.that(panel._peeking, "director_stopped: current implementation does not reset preview flag")
	_assert.eq(panel.choice_shell.position, preview_position, "director_stopped: current implementation does not restore shell geometry")
