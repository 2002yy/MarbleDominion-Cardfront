...
func _test_cardfront_builds_arena_runtime() -> void:
	...
	_assert.that(ai_turret.global_position.y < battlefield_rect.get_center().y, "arena runtime: AI turret should be at the top")
	_assert.that(player_turret.global_position.y > battlefield_rect.get_center().y, "arena runtime: player turret should be at the bottom")
	...
	_assert.that(player_chamber.get_global_bounds_for_test().end.y <= battlefield_rect.end.y, "arena runtime: player chamber should open into the arena")
	_assert.gte(ai_chamber.get_global_bounds_for_test().position.y, battlefield_rect.position.y, "arena runtime: AI chamber should open into the arena")
