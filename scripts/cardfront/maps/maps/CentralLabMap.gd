...
static func make(grid_extent_value) -> Dictionary:
	...
	var min_axis: int = mini(extent.x, extent.y)
	var lab_radius: int = maxi(3, floori(float(min_axis) / 9.0))
	var side_radius: int = maxi(1, floori(float(min_axis) / 20.0))
	var regions: Array = [
		_diamond(center_x, center_y, lab_radius, RegionTypeScript.LAB),
		_rect(center_x - lab_radius - side_radius - 2, center_y - side_radius, center_x - lab_radius - 2, center_y + side_radius, RegionTypeScript.ENERGY),
		_rect(center_x + lab_radius + 2, center_y - side_radius, center_x + lab_radius + side_radius + 2, center_y + side_radius, RegionTypeScript.FACTORY),
	]
	return CardfrontMapDefinitionScript.make("central_lab", extent, regions, {
