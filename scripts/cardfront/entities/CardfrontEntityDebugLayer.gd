...
func setup(new_battlefield, new_registry, new_runtime = null) -> void:
	_disconnect_runtime()
	...
	for entity in registry.entities_by_id.values():
		...
		match str(entity.entity_kind):
			BattlefieldEntityScript.KIND_CREATURE:
				var has_actor: bool = _actors_by_entity_id.has(str(entity.entity_id))
				var creature_scale: float = 1.22 if int(entity.size_slots) >= 2 else 1.0
				var creature_texture: Texture2D = _creature_texture(str(entity.creature_id))
				if has_actor:
					pass
				elif creature_texture != null:
					...
					draw_circle(center, cell_size * 0.30 * creature_scale, Color(0.05, 0.06, 0.08, 0.88))
					draw_circle(center, cell_size * 0.23 * creature_scale, owner_color)
				if str(entity.get("armor_type")) == "armored":
					draw_arc(center, cell_size * 0.32 * creature_scale, 0.0, TAU, 18, Color(1.0, 0.82, 0.30, 0.95), 2.0)
				if int(entity.owner_id) == -1 and creature_texture == null:
...
func _connect_runtime() -> void:
	if runtime == null or not is_instance_valid(runtime):
		return
	var attack_callback := Callable(self, "_on_neutral_creature_attacked")
	var contact_callback := Callable(self, "_on_entity_contact_resolved")
	var removed_callback := Callable(self, "_on_entity_removed")
	if not runtime.neutral_creature_attacked.is_connected(attack_callback):
		runtime.neutral_creature_attacked.connect(attack_callback)
	if not runtime.entity_contact_resolved.is_connected(contact_callback):
		runtime.entity_contact_resolved.connect(contact_callback)
	if not runtime.entity_removed.is_connected(removed_callback):
		runtime.entity_removed.connect(removed_callback)
...
func _disconnect_runtime() -> void:
	...
	var contact_callback := Callable(self, "_on_entity_contact_resolved")
	var removed_callback := Callable(self, "_on_entity_removed")
	if runtime.neutral_creature_attacked.is_connected(attack_callback):
		runtime.neutral_creature_attacked.disconnect(attack_callback)
	if runtime.entity_contact_resolved.is_connected(contact_callback):
		runtime.entity_contact_resolved.disconnect(contact_callback)
	if runtime.entity_removed.is_connected(removed_callback):
		runtime.entity_removed.disconnect(removed_callback)
...
func _on_entity_removed(entity_id: String, _entity_kind: String, _owner_id: int, _cell: Vector2i) -> void:
