...
func setup_static(controller_ref, view_size: Vector2, current_layout: Dictionary = {}, mobile_mode: bool = false) -> void:
	...
	var bar_bg_rect := Rect2(
		top_panel.position + Vector2(14.0, 28.0),
		Vector2(top_panel.size.x - 28.0, 14.0)
	)
...
func _layout_status_labels(view_size: Vector2, mobile_mode: bool, current_layout: Dictionary = {}) -> void:
...
func _load_settings_panel() -> void:
	...
	if ResourceLoader.exists(sp_path):
		...
		print("[CardfrontHUD] Loaded SettingsPanel.tscn")
	else:
		...
		add_child(settings_panel)
		print("[CardfrontHUD] SettingsPanel.tscn not found, fallback to code-generated")
