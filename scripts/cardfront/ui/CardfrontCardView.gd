...
func _create_children() -> void:
	...
	_bg.color = STATE_COLORS["idle"]
	add_child(_bg)

	_hover_border = ColorRect.new()
	_hover_border.name = "HoverBorder"
	_hover_border.size = CARD_SIZE
	_hover_border.position = Vector2.ZERO
	_hover_border.color = Color(0.62, 0.90, 1.0, 0.0)
	add_child(_hover_border)

	_selected_border = ColorRect.new()
	_selected_border.name = "SelectedBorder"
	_selected_border.size = CARD_SIZE - Vector2(2, 2)
	_selected_border.position = Vector2(1, 1)
	_selected_border.color = Color(0.62, 0.90, 1.0, 0.0)
	add_child(_selected_border)

	_icon_rect = ColorRect.new()
	_icon_rect.name = "IconRect"
	_icon_rect.position = Vector2(8, 8)
	_icon_rect.size = Vector2(44, 44)
	_icon_rect.color = Color(0.10, 0.16, 0.28, 0.7)
	add_child(_icon_rect)

	_name_label = Label.new()
	_name_label.name = "NameLabel"
	_name_label.position = Vector2(58, 6)
	...
	_cost_label.position = Vector2(58, 26)
	_cost_label.size = Vector2(116, 18)
	_cost_label.add_theme_font_size_override("font_size", 11)
	_cost_label.add_theme_color_override("font_color", COST_COLORS["affordable"])
	add_child(_cost_label)

	_status_label = Label.new()
	_status_label.name = "StatusLabel"
	_status_label.position = Vector2(8, 54)
	_status_label.size = Vector2(164, 40)
	_status_label.add_theme_font_size_override("font_size", 10)
	_status_label.add_theme_color_override("font_color", Color(0.62, 0.68, 0.80))
	_status_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART as TextServer.AutowrapMode
