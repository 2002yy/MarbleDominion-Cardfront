extends RefCounted
class_name CardfrontHeroBalanceAudit
...
func format_summary(report: Dictionary) -> String:
	...
	lines.append(
		"matches=%d passed=%s median_round=%.1f p90_round=%.1f timeout=%.2f%%" % [
			...
			float((report.get("pacing", {}) as Dictionary).get("p90_round", 0.0)),
			float((report.get("pacing", {}) as Dictionary).get("timeout_rate", 0.0)),
		]
	)
...
func _threshold_report(report: Dictionary) -> Dictionary:
	var failures: Array[String] = []
	for hero_id in (report["hero_rates"] as Dictionary).keys():
		var rate: float = float(((report["hero_rates"] as Dictionary)[hero_id] as Dictionary)["rate"])
		if not ConfigScript.rate_in_range(rate, ConfigScript.AGGREGATE_WIN_RATE_MIN, ConfigScript.AGGREGATE_WIN_RATE_MAX):
			failures.append("aggregate:%s:%.3f" % [str(hero_id), rate])
	for matchup_key in (report["matchup_rates"] as Dictionary).keys():
		var split_key: PackedStringArray = str(matchup_key).split("|")
		if split_key.size() == 2 and split_key[0] == split_key[1]:
			continue
		var rate: float = float(((report["matchup_rates"] as Dictionary)[matchup_key] as Dictionary)["rate"])
		if not ConfigScript.rate_in_range(rate, ConfigScript.MATCHUP_WIN_RATE_MIN, ConfigScript.MATCHUP_WIN_RATE_MAX):
			failures.append("matchup:%s:%.3f" % [str(matchup_key), rate])
	for hero_id in (report["mirror_blue_rates"] as Dictionary).keys():
		var rate: float = float(((report["mirror_blue_rates"] as Dictionary)[hero_id] as Dictionary)["rate"])
		if not ConfigScript.rate_in_range(rate, ConfigScript.MIRROR_BLUE_RATE_MIN, ConfigScript.MIRROR_BLUE_RATE_MAX):
			failures.append("mirror:%s:%.3f" % [str(hero_id), rate])
	var pacing: Dictionary = report["pacing"] as Dictionary
	if not ConfigScript.rate_in_range(float(pacing["median_round"]), ConfigScript.MEDIAN_ROUND_MIN, ConfigScript.MEDIAN_ROUND_MAX):
		failures.append("median_round:%.3f" % float(pacing["median_round"]))
	if float(pacing["p90_round"]) > ConfigScript.P90_ROUND_MAX:
		failures.append("p90_round:%.3f" % float(pacing["p90_round"]))
	if not ConfigScript.rate_in_range(float(pacing["timeout_rate"]), ConfigScript.TIMEOUT_RATE_MIN, ConfigScript.TIMEOUT_RATE_MAX):
		failures.append("timeout_rate:%.3f" % float(pacing["timeout_rate"]))
	var metrics: Dictionary = report["metrics"] as Dictionary
	if not ConfigScript.rate_in_range(
		float(metrics["average_first_stronghold_round"]),
		ConfigScript.FIRST_STRONGHOLD_MIN,
		ConfigScript.FIRST_STRONGHOLD_MAX
	):
		failures.append("first_stronghold:%.3f" % float(metrics["average_first_stronghold_round"]))
	if int(metrics["invalid_offers"]) != 0:
		failures.append("invalid_offers:%d" % int(metrics["invalid_offers"]))
	if int(report["matches"]) != int(report["expected_matches"]):
		failures.append("match_count:%d/%d" % [int(report["matches"]), int(report["expected_matches"])])
